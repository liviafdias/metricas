...
def filtra_instagram():
    ...
    # --- Cálculo da relevância separada ---
    df['RELEVANCIA_CURTIDAS_%'] = ((df['CURTIDAS'] / df.groupby("MARCA_PAIS")['CURTIDAS'].transform('sum') * 100).round(0).astype(int).astype(str) + '%')

    df['RELEVANCIA_COMENTARIOS_%'] = ((df['COMENTARIOS'] / df.groupby("MARCA_PAIS")['COMENTARIOS'].transform('sum') * 100).round(0).astype(int).astype(str) + '%')
    ...
    with ig_tab1:
        ...
        if dados.empty:
            ...
            if not dados.empty and 'LINK_PUBLICACAO' in dados.columns:
                # Normaliza para string e identifica tipo a partir do link
                ...
                # fig_tipo_total = px.bar(
                #     contagem_tipos, x='Tipo de Post', y='Quantidade',
                #     color='Tipo de Post', title="Distribuição de Tipos (Total)"
                # )
                # fig_tipo_total.update_layout(showlegend=False)
                # st.plotly_chart(fig_tipo_total, use_container_width=True)

                # Contagem por Marca-País
                ...
                # Contagem por período (respeita o "Agrupar por": mês/ano)
